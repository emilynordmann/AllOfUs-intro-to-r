...
-   **`File > New Project...`**
-   Name the project `r path("intro-to-r")`
...
knitr::include_graphics(c("images/reports/new_proj_1.png",
...
- Name the project `r path("intro-to-r")` by clicking on the name of the project (by default "Untitled project") at the top.

```{r, echo = FALSE, fig.cap="Starting a new project in the cloud.", fig.show="hold", out.width = "32%"}
knitr::include_graphics(c("images/reports/new_project.png",
                   "images/reports/name_project.png"))
```
...
Once you've opened a new document be sure to save it by clicking **`File > Save As...`**. Name this file something sensible like `chapter_3` (if you are on a Mac and can see the file `r glossary("extension")`, name it `chapter_3.Rmd`). This file will automatically be saved in your project folder, i.e., your working directory, so you should now see this file appear in your file viewer pane.
...
Type the name of a data set into the `r glossary("console")` to see the data. For example, type `?covid_testing` into the console to see the dataset description for `covid_testing`, which is a data set included with `r pkg("medicaldata")`.
...
x_breaks <- seq(0,90,10)
y_breaks <- seq(0,1250000, 25000)

a <- ggplot() + labs(subtitle = "Layer 1")
b <- ggplot(gapminder, aes(x = lifeExp, y = gdpPercap)) + 
  labs(subtitle = "Layer 2")
c <- b + geom_point() + 
  labs(subtitle = "Layer 3")
d <- c + geom_smooth(method = "loess") + 
  labs(subtitle = "Layer 4")
e <- d + scale_x_continuous(name = "Life Expectancy", breaks = x_breaks) +
  scale_y_continuous(name = "GDP per capita", breaks = y_breaks) + 
...
```{r remove-layer, fig.cap="Final plot with scatterplot layer removed.", echo = FALSE, message=FALSE}
ggplot(gapminder, aes(x = lifeExp, y = gdpPercap))+
  geom_smooth(method = "loess") +
  scale_x_continuous(name = "Life Expectancy", breaks = x_breaks) +
  scale_y_continuous(name = "GDP per capita", breaks = y_breaks) + ggthemes::theme_gdocs(base_size = 11) + 
  theme(axis.line.x = element_blank(),
        plot.background = element_blank()) +
  labs(subtitle = "Layer 6") +
  theme(plot.subtitle =  element_text(color = "black"))
