...
When you installed R, that gave your computer the ability to process the R programming language, and also installed an app called "R". We will never use that app. Instead, we will use [RStudio](http://www.rstudio.com){target="_blank"}.  RStudio is arranged with up to four window `r glossary("panes")`.
...
Once you have the `r pkg("devtools")` package, you can also install packages from repositories other than CRAN, such as github. The following code installs the package associated with a [paper Emily wrote on data visualization](https://journals.sagepub.com/doi/full/10.1177/25152459221074654).
...
`r pkg("tidyverse")`is a meta-package that loads several packages we'll be using in every chapter in this course:
